...
public class SingleExtensionEvalFunction implements SkyFunction {
          ...
          .post(
              ModuleExtensionResolutionEvent.create(
                  ...
                  LockFileModuleExtension.create(
                      bzlTransitiveDigest, moduleExtensionResult.getAccumulatedFileDigests(), extensionEnvVars, generatedRepoSpecs)));
    }
  ...
  private SingleExtensionEvalValue tryGettingValueFromLockFile(
      Environment env, ModuleExtensionId extensionId,
    ...
    Boolean filesChanged = isFilesChanged(env, lockedExtension.getAccumulatedFileDigests());
  ...
  private Boolean isFilesChanged(Environment env, ImmutableMap<Label, String> accumulatedFileDigests)
      throws InterruptedException {
    ImmutableMap<Label, FileValue.Key> fileKeys = accumulatedFileDigests.keySet().stream()
        .collect(ImmutableMap.toImmutableMap(
            label -> label, label -> {
              RootedPath filePath = RootedPath.toRootedPath(
                  Root.fromPath(directories.getWorkingDirectory()),
                  label.toPathFragment()
              );
              return FileValue.key(filePath);
            }
        ));
    //Compare the collected with the hashes stored in the lockfile
    SkyframeLookupResult result = env.getValuesAndExceptions(fileKeys.values());
    if (env.valuesMissing()) {
      return null;
    }

    for(ImmutableMap.Entry<Label, String> entry: accumulatedFileDigests.entrySet()){
      FileValue fileValue = (FileValue) result.get(fileKeys.get(entry.getKey()));
      if(!entry.getValue().equals(fileValue.getDigest())) {
        return true;
      }
    }
    return false;
  ...
  private RunModuleExtensionResult runModuleExtension(
    ...
    return RunModuleExtensionResult.create(moduleContext.getAccumulatedFileDigests(),
...
@AutoValue
abstract class RunModuleExtensionResult {

  public static RunModuleExtensionResult create(
      ImmutableMap<Label, String> accumulatedFileDigests,
      ImmutableMap<String, RepoSpec> generatedRepoSpecs) {
    return new AutoValue_RunModuleExtensionResult(accumulatedFileDigests, generatedRepoSpecs);
  }
